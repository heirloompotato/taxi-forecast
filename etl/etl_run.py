...
def run_etl():
    try:
        ...
        logging.info("Loading config tables from GCS")
        ...
        logging.info("Config tables loaded successfully")
        ...
        bq_snapshot = etl_utils.pull_recent_data_bq(bq_client, bq_table_id, cutoff)
        ...
        overwritten_records = etl_utils.overwrite_recent_values_with_time_regressors(bq_snapshot, merged_records, time_dim)
        ...
        # Forecast num taxis
        forecasted_records = forecast_utils.forecast_num_taxis(ffilled_records)

        # Update BigQuery table (delete + insert)
        logging.info("Updating BigQuery table")
        etl_utils.update_bq_data(bq_client, bq_table_id, forecasted_records, cutoff)
